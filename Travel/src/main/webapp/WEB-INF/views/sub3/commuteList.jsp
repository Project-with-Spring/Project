	          ...
	          <table class="table table-striped table-responsive tbl_narrow">
	            ...
	            <thead>
	                <tr>
	  				...
	  				<tbody>
	  				<c:choose>
					    <c:when test="${fn:length(staffCommutfList) == 0}">
				        <tr>
					        <td colspan="11"  class="tac">조회된 직원이 없습니다.</td>
							</tr>
					    </c:when>
					    <c:otherwise>
					        <c:forEach var="stcl" items="${staffCommutfList}" varStatus="status">
					        	<tr>
					        		<td class="tac">${status.index+1}</td>
					        		<td class="tac">${stcl.stf_id} <!-- 직원ID 출력 --></td>
					        		<td class="tac">${stcl.stf_name} <!-- 직원이름 출력 --></td>
					        		<td class="tac">${stcl.pst_name} <!-- 직급 출력 --></td>
					        		<td class="tac">${stcl.cmt_go} <!-- 직원이름 출력 --></td>
					        		<td class="tac">${stcl.cmt_leave} <!-- 직원이름 출력 --></td>
					        		<td class="text-right">					        			
